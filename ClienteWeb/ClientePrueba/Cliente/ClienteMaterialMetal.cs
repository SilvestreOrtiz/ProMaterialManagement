using ClienteWeb.MaterialMetales;
using System;
...
namespace ClientePrueba.Cliente
{
    public class ClienteMaterialMetal
    {
        ...
        public void subir(ClienteWeb.MaterialMetales.Metales metales)
        {

            DataContractJsonSerializer ser = new DataContractJsonSerializer(typeof(ClienteWeb.MaterialMetales.Metales));
        ...
        public ClienteWeb.MaterialMetales.Metales encontrarUno(string id_metal)
        {
            var synClient = new WebClient();
            string url = string.Format(BASE_URL + "get/{0}", id_metal);
            var content = synClient.DownloadString(url);
            var json_serializer = new JavaScriptSerializer();
            return json_serializer.Deserialize<ClienteWeb.MaterialMetales.Metales>(content);

        }

        //Agrega una fila a la tabla
        public void editar(ClienteWeb.MaterialMetales.Metales metales)
        {
        ...
        public void eliminar(ClienteWeb.MaterialMetales.Metales metales)
        {
